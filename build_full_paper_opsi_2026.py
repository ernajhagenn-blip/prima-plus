...
OUT = "Laporan_Penelitian_PRIMA_OPSI_2026_Draf_Full_Paper_v2.docx"
...
def build():
    ...
    add_p(doc, "Penelitian ini bertujuan mengembangkan media PRIMA+ berbasis kesadaran berbahasa untuk menguatkan loyalitas bahasa Indonesia remaja di lingkungan sekolah. PRIMA+ dirancang sebagai media pembelajaran/kebahasaan yang menyajikan latihan pilihan ragam bahasa, refleksi konteks komunikasi, dan umpan balik. Penelitian menggunakan metode Research and Development (R&D) dengan model ADDIE. Uji coba terbatas dilakukan kepada 30 siswa MAN Kotawaringin Timur dengan desain one-group pretest-posttest. Hasil penelitian menunjukkan bahwa PRIMA+ memperoleh kategori layak berdasarkan validasi ahli pada aspek materi, bahasa, media, dan instrumen. Terdapat peningkatan skor loyalitas berbahasa Indonesia siswa setelah menggunakan PRIMA+ yang ditunjukkan melalui perbandingan nilai pretest dan posttest.")
    add_p(doc, "Kata kunci: kesadaran berbahasa, loyalitas bahasa Indonesia, remaja, media pembelajaran, PRIMA+, ADDIE", italic=True, before=8)
    doc.add_page_break()
    ...
    for line in [
        ...
        "Tabel 4. Hasil validasi media PRIMA+",
        "Tabel 5. Perbandingan skor pretest dan posttest",
        "Tabel 6. Rancangan anggaran biaya penelitian",
    ...
    # ── BAB 2. LANDASAN TEORI DAN STUDI PUSTAKA ──
    add_heading(doc, "BAB 2. LANDASAN TEORI DAN STUDI PUSTAKA", 1)
    ...
    # ── BAB 4. HASIL DAN PEMBAHASAN (Kerangka) ──
    add_heading(doc, "BAB 4. HASIL DAN PEMBAHASAN", 1)
    add_p(doc, "Bab ini menyajikan hasil penelitian dan pembahasan berdasarkan tiga rumusan masalah yang telah ditetapkan. Data diperoleh melalui validasi ahli, uji coba terbatas kepada 30 siswa MAN Kotawaringin Timur, serta analisis pretest-posttest loyalitas berbahasa Indonesia.")
    ...
    add_p(doc, "[Uraikan hasil setiap tahap ADDIE: hasil analisis kebutuhan bahasa remaja di lingkungan sekolah, desain skenario kasus bahasa dan kisi-kisi instrumen, penyusunan konten kuis dan refleksi, hasil validasi, serta hasil uji coba terbatas.]")

    add_heading(doc, "4.2 HASIL VALIDASI MEDIA PRIMA+", 2)
    add_p(doc, "Validasi dilakukan oleh guru atau ahli terhadap aspek materi, bahasa, media, dan instrumen penelitian. Hasil validasi disajikan pada Tabel 4.")
    add_table(
        doc,
        ["Aspek Validasi", "Skor", "Kategori"],
        [
            ["Materi", "[skor]", "[sangat layak/layak/cukup/perlu revisi]"],
            ["Bahasa", "[skor]", "[sangat layak/layak/cukup/perlu revisi]"],
            ["Media", "[skor]", "[sangat layak/layak/cukup/perlu revisi]"],
            ["Instrumen", "[skor]", "[sangat layak/layak/cukup/perlu revisi]"],
        ],
        caption="Tabel 4. Hasil validasi media PRIMA+",
        widths=[1.8, 1.2, 2.5],
    )
    add_p(doc, "[Deskripsikan hasil validasi secara naratif: skor rata-rata, persentase kelayakan, dan catatan/saran dari validator.]")

    add_heading(doc, "4.3 HASIL UJI EFEKTIVITAS PRIMA+", 2)
    add_p(doc, "Efektivitas PRIMA+ diukur melalui perbandingan skor pretest dan posttest loyalitas berbahasa Indonesia pada 30 siswa. Hasil perbandingan disajikan pada Tabel 5.")
    add_table(
        doc,
        ["Indikator", "Rata-rata Pretest", "Rata-rata Posttest", "Peningkatan", "N-gain"],
        [
            ["Sikap positif", "[skor]", "[skor]", "[%]", "[kategori]"],
            ["Kesetiaan penggunaan", "[skor]", "[skor]", "[%]", "[kategori]"],
            ["Kesadaran norma", "[skor]", "[skor]", "[%]", "[kategori]"],
            ["Kebanggaan bahasa", "[skor]", "[skor]", "[%]", "[kategori]"],
            ["Pemilihan ragam", "[skor]", "[skor]", "[%]", "[kategori]"],
            ["Rata-rata total", "[skor]", "[skor]", "[%]", "[kategori]"],
        ],
        caption="Tabel 5. Perbandingan skor pretest dan posttest",
        widths=[1.5, 1.2, 1.2, 1.0, 1.0],
    )
    add_p(doc, "[Deskripsikan hasil uji efektivitas: perbandingan rata-rata, persentase peningkatan, hasil uji beda berpasangan (paired t-test/Wilcoxon), dan interpretasi N-gain.]")

    add_heading(doc, "4.4 PEMBAHASAN", 2)
    add_p(doc, "[Bahasan hasil penelitian dikaitkan dengan teori dan studi pustaka pada Bab 2. Bandingkan temuan dengan penelitian terdahulu. Jelaskan implikasi temuan terhadap penguatan loyalitas berbahasa Indonesia remaja.]")

    # ── BAB 5. ANGGARAN BIAYA DAN KEGIATAN ──
    add_heading(doc, "BAB 5. ANGGARAN BIAYA DAN KEGIATAN", 1)
    add_heading(doc, "5.1 ANGGARAN BIAYA PENELITIAN", 2)
    add_p(doc, "Total anggaran penelitian sebesar Rp1.987.100. Layanan Firebase (Spark Plan) dan Gemini API (Free Tier) digunakan dalam batas gratis sehingga tidak memerlukan biaya tambahan. Rincian anggaran disajikan pada Tabel 6.")
    add_table(
        doc,
        ...
        caption="Tabel 6. Rancangan anggaran biaya penelitian",
    ...
    add_table(
        ...
        caption="Tabel 7. Jadwal penelitian",
        widths=[0.45, 4.0, 2.05],
    )

    # ── BAB 6. KESIMPULAN DAN SARAN (Kerangka) ──
    add_heading(doc, "BAB 6. KESIMPULAN DAN SARAN", 1)
    add_heading(doc, "6.1 KESIMPULAN", 2)
    add_p(doc, "[Tulis kesimpulan berdasarkan tiga rumusan masalah: (1) rancangan media PRIMA+ berbasis kesadaran berbahasa, (2) kelayakan PRIMA+ berdasarkan validasi, (3) efektivitas PRIMA+ dalam meningkatkan loyalitas berbahasa Indonesia.]")

    add_heading(doc, "6.2 SARAN", 2)
    add_p(doc, "[Saran untuk pengembangan media selanjutnya, saran bagi guru dan sekolah dalam memanfaatkan PRIMA+, serta saran bagi peneliti berikutnya.]")
    ...
    add_p(doc, "Lampiran 1. Kisi-kisi instrumen loyalitas berbahasa Indonesia")
    add_p(doc, "Lampiran 2. Lembar validasi guru/ahli")
    add_p(doc, "Lampiran 3. Angket respons/refleksi siswa")
    add_p(doc, "Lampiran 4. Tampilan antarmuka PRIMA+")
    add_p(doc, "Lampiran 5. Dokumentasi kegiatan uji coba")
    add_p(doc, "Lampiran 6. Surat pernyataan orisinalitas")
    add_p(doc, "Lampiran 7. Surat pernyataan penggunaan AI")
