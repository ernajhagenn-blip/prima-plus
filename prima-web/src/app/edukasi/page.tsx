...
export default function EdukasiPage() {
  ...
  return (
    <main style={{ width: "100vw", minHeight: "100vh", margin: 0, position: "relative", zIndex: 1, background: "#0a0e1a", padding: "clamp(16px,4vmin,48px)", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ width: "100%", maxWidth: 700, display: "flex", flexDirection: "column", gap: 18 }}>
        <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 18, padding: "20px 24px", border: "1px solid rgba(255,255,255,0.15)", animation: "rise 0.5s ease both" }}>
          <p style={{ fontFamily: "'Nunito','Poppins',Arial,sans-serif", fontSize: 15, color: "#ffffff", margin: 0, lineHeight: 1.75, fontWeight: 500 }}>{T.p1}</p>
          <p style={{ fontFamily: "'Nunito','Poppins',Arial,sans-serif", fontSize: 15, color: "#ffffff", margin: "14px 0 0", lineHeight: 1.75, fontWeight: 500 }}>{T.p2}</p>
        </div>

        <div style={{ background: "rgba(124,58,237,0.15)", borderRadius: 18, padding: "20px 24px", border: "1px solid rgba(124,58,237,0.5)", animation: "rise 0.5s 0.12s ease both" }}>
          <h2 style={{ fontFamily: "'Righteous',sans-serif", fontSize: "clamp(16px,3vmin,20px)", color: "#d8b4fe", margin: "0 0 14px", textShadow: "0 2px 8px rgba(124,58,237,0.3)" }}>{T.s1}</h2>
          <p style={{ fontFamily: "'Nunito','Poppins',Arial,sans-serif", fontSize: 15, color: "#ffffff", margin: 0, lineHeight: 1.75, fontWeight: 500 }}>{T.p3}</p>
          <p style={{ fontFamily: "'Nunito','Poppins',Arial,sans-serif", fontSize: 15, color: "#ffffff", margin: "14px 0 0", lineHeight: 1.75, fontWeight: 500 }}>{T.p4}</p>
        </div>

        <div style={{ background: "rgba(14,165,233,0.12)", borderRadius: 18, padding: "20px 24px", border: "1px solid rgba(14,165,233,0.5)", animation: "rise 0.5s 0.24s ease both" }}>
          <h2 style={{ fontFamily: "'Righteous',sans-serif", fontSize: "clamp(16px,3vmin,20px)", color: "#7dd3fc", margin: "0 0 14px", textShadow: "0 2px 8px rgba(14,165,233,0.3)" }}>{T.s2}</h2>
          <p style={{ fontFamily: "'Nunito','Poppins',Arial,sans-serif", fontSize: 15, color: "#ffffff", margin: 0, lineHeight: 1.75, fontWeight: 500 }}>{T.p5}</p>
          <div style={{ margin: "14px 0 0", display: "flex", flexDirection: "column", gap: 10 }}>
            {[T.li1, T.li2].map((li, i) => (
              <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                <span style={{ flexShrink: 0, width: 24, height: 24, borderRadius: "50%", background: "rgba(56,189,248,0.3)", color: "#7dd3fc", fontFamily: "'Righteous',sans-serif", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 2 }}>✓</span>
                <p style={{ fontFamily: "'Nunito','Poppins',Arial,sans-serif", fontSize: 14.5, color: "#ffffff", margin: 0, lineHeight: 1.65, fontWeight: 500 }}>{li}</p>
              </div>
            ))}
          </div>
        </div>

        <div style={{ background: "rgba(250,204,21,0.1)", borderRadius: 18, padding: "20px 24px", border: "1px solid rgba(250,204,21,0.5)", animation: "rise 0.5s 0.36s ease both" }}>
          <h2 style={{ fontFamily: "'Righteous',sans-serif", fontSize: "clamp(16px,3vmin,20px)", color: "#fde047", margin: "0 0 14px", textShadow: "0 2px 8px rgba(250,204,21,0.3)" }}>{T.s3}</h2>
          <p style={{ fontFamily: "'Nunito','Poppins',Arial,sans-serif", fontSize: 15, color: "#ffffff", margin: "0 0 16px", lineHeight: 1.75, fontWeight: 500 }}>{T.p6}</p>
          <div style={{ display: "grid", gap: 10 }}>
            {[
              ...
              <div key={p.n} style={{ display: "flex", gap: 12, alignItems: "flex-start", background: "rgba(0,0,0,0.3)", borderRadius: 14, padding: "14px 16px", border: "1px solid rgba(255,255,255,0.08)" }}>
                <span style={{ flexShrink: 0, width: 32, height: 32, borderRadius: "50%", background: p.c, color: "#0b0d22", fontFamily: "'Righteous','Arial Black',sans-serif", fontSize: 15, fontWeight: 900, display: "flex", alignItems: "center", justifyContent: "center" }}>{p.n}</span>
                <p style={{ fontFamily: "'Nunito','Poppins',Arial,sans-serif", fontSize: 14, color: "#ffffff", margin: 0, lineHeight: 1.65, fontWeight: 500 }}>{p.t}</p>
              </div>
            ))}
          </div>
        </div>
