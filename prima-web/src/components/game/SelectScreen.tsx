...
export default function SelectScreen() {
  ...
  return (
    <main style={{ width: "100vw", minHeight: "100vh", margin: 0, background: "radial-gradient(ellipse at 50% -10%, #312e81 0%, #1e1b4b 40%, #0b0d22 80%)", padding: "clamp(14px,3vmin,36px) clamp(12px,3vmin,32px)", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ width: "100%", maxWidth: 900, display: "flex", flexWrap: "wrap", justifyContent: "center", alignItems: "stretch", gap: "clamp(10px,2.5vmin,28px)", marginTop: 8 }}>
        <div style={{ flex: "1 1 340px", minWidth: 300, maxWidth: 460 }}>
          ...
          <h2 key={kart.key} style={{ fontFamily: "'Righteous','Arial Black',sans-serif", fontSize: "clamp(26px,5vmin,40px)", color: kart.accent, textAlign: "center", margin: "14px 0 2px", textShadow: "0 3px 0 #0b0d22, 0 0 26px rgba(255,255,255,0.25)", animation: "kartIn 0.35s cubic-bezier(0.34,1.56,0.64,1) both", letterSpacing: "0.02em" }}>
            {kart.name.toUpperCase()}
          </h2>
          <p key={kart.key + "t"} style={{ fontFamily: "Arial,sans-serif", fontSize: "clamp(13px,2.2vmin,15.5px)", color: "rgba(255,255,255,0.85)", textAlign: "center", margin: "0 auto 16px", maxWidth: 380, lineHeight: 1.55, textShadow: "0 1px 4px rgba(0,0,0,0.8)", animation: "kartIn 0.4s 0.08s ease both" }}>
            {kart.trait}
          </p>
            ...
            {STAT_META.map((m, i) => (
              <div key={m.key} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: i < STAT_META.length - 1 ? 8 : 0 }}>
                ...
                <div style={{ flex: 1, height: 9, borderRadius: 5, background: "rgba(255,255,255,0.1)", overflow: "hidden" }}>
                  <div style={{ width: `${kart.stats[m.key] * 10}%`, height: "100%", borderRadius: 5, background: m.color, boxShadow: `0 0 8px ${m.color}88`, transition: "width 0.4s cubic-bezier(0.34,1.56,0.64,1)" }} />
                </div>
        ...
        <div style={{ flex: "1 1 300px", minWidth: 280, maxWidth: 400, display: "flex", flexDirection: "column", justifyContent: "center", gap: 12 }}>
          {KARTS.map((k, i) => {
            ...
            return (
              <button
                key={k.key}
                onClick={() => { setIdx(i); setKart(k.key); }}
                style={{
      ...
      <div style={{ display: "flex", gap: 12, marginTop: "clamp(16px,3vmin,28px)", width: "100%", maxWidth: 480, padding: "0 8px" }}>
        ...
        <button
          onClick={() => router.push("/games/language-kart")}
          style={{ flex: 1, padding: "14px 0", borderRadius: 14, background: "linear-gradient(135deg,#16a34a,#4ade80)", border: "2px solid rgba(255,255,255,0.5)", color: "white", fontFamily: "'Righteous','Arial Black',sans-serif", fontSize: "clamp(15px,2.8vmin,18px)", fontWeight: 900, cursor: "pointer", boxShadow: "0 8px 26px rgba(34,197,94,0.45)", letterSpacing: "0.04em" }}
        >
          GAS KE LINTASAN ▶
        </button>
      </div>
