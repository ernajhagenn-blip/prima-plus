    ...
    <div className="mx-auto max-w-3xl px-4 py-10">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">Data responden dikumpulkan otomatis ke Google Sheets.</p>
        </div>
        <form action={logout}>
          <button type="submit" className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-100">
            Keluar
          </button>
        </form>
      </div>
      ...
      <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-gray-600">
          Jalankan <code className="rounded bg-gray-100 px-1 font-mono">npm run dev</code> di komputer lokal, lalu akses link berikut:
        </p>
        <div className="mt-4 flex flex-wrap gap-2">
          {DATASETS.map((ds) => (
            <a key={ds.key} href={`/api/export?dataset=${ds.key}`} className="inline-block rounded-lg bg-red-700 px-4 py-2 text-sm font-semibold text-white transition hover:bg-red-800">
              {ds.label}
            </a>
          ))}
        </div>
      ...
      <div className="rounded-2xl border border-blue-200 bg-blue-50 p-6">
        ...
        <ol className="mt-3 space-y-2 text-sm text-blue-800">
          ...
          <li><strong>3.</strong> Untuk export lokal: jalankan <code className="rounded bg-blue-100 px-1">npm run dev</code> lalu buka <code className="rounded bg-blue-100 px-1">localhost:3000/admin</code></li>
        </ol>
      </div>
    </div>
  );
